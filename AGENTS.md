...
> For the _why_ behind any rule, see `docs/puretask-decisions.md`.
...
- **Comments:** explain _why_, not _what_. JSDoc for public-API functions.
...
- Validation schemas live in `src/features/<feature>/validation.ts` and are imported by both the form _and_ the server action.
