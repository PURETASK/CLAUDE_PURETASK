...
-- Drop the old constraint first (we'll recreate after adding the columns).
ALTER TABLE bookings DROP CONSTRAINT no_cleaner_double_booking;

-- Add generated columns that include the 60-minute buffer.
-- STORED means Postgres physically writes these; required for use in
-- exclusion constraints (which need indexable expressions).
ALTER TABLE bookings
    ADD COLUMN blocked_start_at TIMESTAMPTZ
        GENERATED ALWAYS AS (start_at - INTERVAL '60 minutes') STORED,
    ADD COLUMN blocked_end_at TIMESTAMPTZ
        GENERATED ALWAYS AS (end_at + INTERVAL '60 minutes') STORED;

COMMENT ON COLUMN bookings.blocked_start_at IS
    'Generated: start_at minus 60-min buffer. Used by no_cleaner_double_booking exclusion constraint.';
COMMENT ON COLUMN bookings.blocked_end_at IS
    'Generated: end_at plus 60-min buffer. Used by no_cleaner_double_booking exclusion constraint.';

-- Recreate the constraint using the buffered ranges.
